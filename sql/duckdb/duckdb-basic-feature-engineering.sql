...
-- the following options requires internet access
-- option 1: copy remote csv directly to database
CREATE OR REPLACE TABLE financial_trx AS
    FROM read_csv('https://blobs.duckdb.org/data/financial_fraud_detection_dataset.csv');

-- option2: copy remote csv file to local csv file and then import local file to db
-- COPY (SELECT * FROM 'https://blobs.duckdb.org/data/financial_fraud_detection_dataset.csv') TO '[[__DATAFOLDER__]]\financial_fraud_detection_dataset.csv' (HEADER, DELIMITER ',');
-- CREATE OR REPLACE TABLE financial_trx AS
    -- FROM read_csv('[[__DATAFOLDER__]]\financial_fraud_detection_dataset.csv');

-- RESULT:count(1)
-- RESULT:5000000
select count(1) from financial_trx;
