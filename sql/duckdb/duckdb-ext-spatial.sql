...
CREATE OR REPLACE TABLE zones AS
    ...
    FROM ST_Read('[[__DATAFOLDER__]]\spatial\nyc_taxi\taxi_zones\taxi_zones.shx');
...
COPY (
    SELECT 
        ST_MakeLine(pickup_point, dropoff_point).ST_FlipCoordinates().ST_AsWKB() AS wkb_geometry,
        start_zone,
        end_zone,
    ...
    order by trip_time
    limit 10
) TO '[[__DATAFOLDER__]]\joined.geojsonseq' 
WITH ( 
    FORMAT gdal, 
    DRIVER 'GeoJSONSeq',
...
-- RESULT:Feature,{'start_zone': Bloomingdale, 'end_zone': Upper West Side South, 'trip_time': '00:00:00'},{'type': LineString, 'coordinates': [[-73.967687, 40.802958], [-73.980353, 40.783393]]}
...
SELECT *
FROM read_json('[[__DATAFOLDER__]]\joined.geojsonseq')
order by properties;