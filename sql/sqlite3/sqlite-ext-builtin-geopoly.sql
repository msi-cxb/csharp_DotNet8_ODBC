...
-- =============================================================
-- Test: Geopoly Extension - Polygon Overlap and Within Queries
-- Description: Verifies the geopoly virtual table module (built in
--              to the SQLiteODBC driver). Sets up three staging
--              tables (basis, xform, xyoff) with 8 polygon shapes,
--              4 transformation matrices, and 160 placement offsets,
--              then creates a geopoly virtual table (geo1) by
--              applying geopoly_xform to each shape. Exercises:
--                - geopoly_overlap: polygons intersecting a query region
--                - geopoly_within:  polygons fully inside a query region
--                - geopoly_regular: regular polygon generation
--                - geopoly_blob/json/svg: serialisation formats
--                - geopoly_area:    polygon area calculation
--              No RESULT annotations — the overlap/within row counts
--              vary with the random placement data, and the geometry
--              output values are not compared in the original test.
-- Source: sqlite_extension_geopoly in sqliteODBC_tests.vbs (line 3705)
-- Reference: https://sqlite.org/geopoly.html
--            https://sqlite.org/src/file/ext/rtree/visual01.txt
-- Connection: sqlite3 (geopoly is built into the SQLiteODBC driver)
-- =============================================================
...
.print === Create basis table (8 named polygon shapes) ===
...
.print === Create xform table (4 rotation/colour transforms) ===
...
.print === Create xyoff table (160 placement offsets) ===
...
.print === Build geopoly virtual table from shape + transform + offset data ===

DROP TABLE IF EXISTS geo1;
...
.print === Create query polygon (irregular 10-vertex polygon) ===
...
.print === geopoly_overlap — shapes that overlap the query polygon ===

-- No RESULT annotation: row count depends on placement data
SELECT geo1._shape, geo1.type, geo1.clr
FROM geo1, querypoly
WHERE geopoly_overlap(_shape, poly);

.print === geopoly_within — shapes fully within the query polygon ===
...
.print === geopoly_regular — equilateral triangle at (100,100) with circumradius 40 ===

-- RESULT:p
-- RESULT:X'01000003B1000C430000C842F500A042BDA40643F500A04286B68242'
SELECT geopoly_blob(geopoly_regular(100, 100, 40, 3)) AS p;

-- RESULT:p
-- RESULT:[[140.003,100.0],[80.0019,134.644],[80.0019,65.3565],[140.003,100.0]]
SELECT geopoly_json(geopoly_regular(100, 100, 40, 3)) AS p;

-- RESULT:p
-- RESULT:<polyline points='140.003,100 80.0019,134.644 80.0019,65.3565 140.003,100'></polyline>
SELECT geopoly_svg(geopoly_regular(100, 100, 40, 3)) AS p;

.print === geopoly_area — area of the equilateral triangle ===

-- RESULT:a
-- RESULT:2078.63916015625
SELECT geopoly_area(geopoly_regular(100, 100, 40, 3)) AS a;
